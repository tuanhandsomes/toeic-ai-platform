  ...
  async importBundle(bundle, createdBy) {
    ...
    if (existing) {
      throw ApiError.conflict(
        `Đề "${testInfo.title}" đã tồn tại trong DB. Xóa đề cũ trước khi import lại.`,
      );
    }

    // ─── 1. Build question docs with auto-filled URLs ────────────────────
    const seriesTag = (testInfo.series || 'ets-2026').toLowerCase().replace(/\s+/g, '-');
    const questionsToInsert = rawQuestions.map((q) => {
      ...
      return {
        ...
        content: {
          text: q.text || '',
          audioUrl: q.audioUrl !== undefined ? q.audioUrl : buildAudioUrl(qNum, testCode, folder),
          imageUrl: q.imageUrl !== undefined ? q.imageUrl : buildPart1ImageUrl(qNum, folder),
          passageId: null,
        },
        options: q.options,
        ...
        tags: Array.from(new Set([...(q.tags || []), seriesTag, `part${q.part}`])),
      };
    });

    // ─── 2. Insert all questions in one batch ────────────────────────────
    const inserted = await Question.insertMany(questionsToInsert, { ordered: true });
    ...
    const ordered = inserted
      .map((q, idx) => ({ id: q._id, num: rawQuestions[idx].questionNumber, part: q.part }))
    ...
    const fullTest = await Test.create({
      ...
      description:
        testInfo.description || `Đề thi TOEIC full ${testInfo.year || ''} — 200 câu, 120 phút.`,
      type: 'full',
      part: null,
      questionIds: fullIds,
      durationMinutes: 120,
