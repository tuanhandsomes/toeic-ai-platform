...
async function seed(jsonFilename) {
  ...
  const questionsToInsert = rawQuestions.map((q) => {
    ...
    return {
      ...
      tags: [...(q.tags || []), testInfo.series.toLowerCase().replace(/\s+/g, '-'), `part${q.part}`],
    };
  });
  ...
  const fullTest = await Test.create({
    title: testInfo.title,
    description: testInfo.description || `Đề thi TOEIC full ${testInfo.year || ''} — 200 câu, 120 phút.`,
    type: 'full',
    part: null,
  ...
  console.log(`✓ Created Full Test: ${fullTest.title} (${orderedIds.length} câu)`);

  // 6. Create 7 Practice Sets (1 per part) — REUSE same questionIds
  const PART_META = {
  ...
  for (let p = 1; p <= 7; p++) {
    ...
    console.log(`✓ Created Practice: ${practiceTest.title} (${partQuestions.length} câu)`);
  }
